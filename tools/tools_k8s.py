    ...
    try:
        ...
        if pod.spec.volumes:
            ...
            for v in pod.spec.volumes:
                ...
                detail = ""
                if v.secret:
                    detail = f"  Secret: {v.secret.secret_name or '?'}"
                elif v.config_map:
                    detail = f"  ConfigMap: {v.config_map.name or '?'}"
                elif v.persistent_volume_claim:
                    detail = f"  PVC: {v.persistent_volume_claim.claim_name or '?'}"
                elif v.empty_dir is not None:
                    detail = "  EmptyDir"
                elif v.projected:
                    detail = "  Projected"
                elif v.host_path:
                    detail = f"  HostPath: {v.host_path.path or '?'}"
                elif v.csi:
                    detail = f"  CSI: {v.csi.driver or '?'}"
                lines.append(f"  {v.name}: {vtype}{(' — ' + detail.strip()) if detail else ''}")
...
def get_endpoints(namespace: str = "all", search: str | None = None, port: int = 0) -> str:
    try:
                    ...
                    for addr in (subset.addresses or []):
                        for p in (subset.ports or []):
                            if port and p.port != port:
                                continue
                            rows.append((ep.metadata.namespace, ep.metadata.name, f"{addr.ip}:{p.port}"))
            return rows
        table_rows = _ep_rows(eps_list, True)
        if search and not table_rows:
            table_rows = _ep_rows(eps_list, False)
        if not table_rows:
            return (f"No active Endpoints found on port {port}." if port
                    else "No active Endpoints (no addresses) found.")
        header = f"Endpoints on port {port}" if port else "Endpoints"
        md_lines = [_ns_header(header, namespace, search)]
        if namespace == "all":
...
def get_service(namespace: str = "all", search: str = None, port: int = 0) -> str:
    try:
        ...
        def _rows(items, apply_search):
            ...
            for svc in items:
                ...
                if port and not any(p.port == port or p.target_port == port for p in svc_ports): continue
                ports    = ", ".join(f"{p.port}/{p.protocol}" for p in svc_ports)
        ...
        if port and not rows:
            return f"No services found exposing port {port}."
        header = f"Services exposing port {port}" if port else None
        md_lines = [_ns_header(header or "Services", namespace, search)]
        if namespace == "all":
...
def _enrich_cml_usernames(namespaces: set) -> dict:
    mapping = {}
    base_namespaces = {ns.split("-user-")[0] for ns in namespaces if "-user-" in ns}
        ...
        try:
            ...
            if sep_idx != -1:
                for line in lines[sep_idx+1:]:
                    parts = line.split("|")
                    if len(parts) >= 2:
                        ns_val = parts[0].strip()
                        usr_val = parts[1].strip()
                        if ns_val and usr_val:
                            mapping[ns_val] = f"{ns_val} ({usr_val})"
        except Exception as e:
            _log.warning(f"Failed to enrich usernames for {base_ns}: {e}")
...
def get_workbench_top_requests(namespace: str, limit: int = 10, sort_by: str = "cpu",
    ...
    if m:
        ...
        sql_interval = f"{val} {unit_map[unit]}"
    else:
    ...
    sql = (
        ...
        f"FROM {table_name} d "
        f"LEFT JOIN users u ON u.namespace = '{namespace}-user-' || d.creator_id "
        f"WHERE d.created_at >= NOW() - INTERVAL '{sql_interval}'{search_cond} "
        f"ORDER BY d.{sort_col} DESC "
        f"LIMIT {limit}"
    )
    ...
    if data_start > 0:
        for line in lines[data_start:]:
            ...
            if len(parts) >= 7:
                u_ns, uname, wname, cpu_str, mem_str, status, ts_str = parts[:7]
                try:
    ...
    for r in parsed_rows:
        ...
        # Table Row
        uname_display = uname if uname else "-"
        md_lines.append(f"{u_ns:<{col_ns}}  {uname_display:<{col_user}}  {wname:<{col_wl}}  {cpu_val:>6g}  {mem_val:>8g}  {status:<10}  {clean_ts}")
        ...
        val = cpu_val if sort_col == "cpu" else mem_val
        label = uname if uname else u_ns
        
        if label not in series_dict:
            series_dict[label] = []
        series_dict[label].append([ts, val])
...
def get_top_pods(namespace: str = "all", limit: int = 5, sort_by: str = "cpu",
    ...
    # Determine memory formatting for the live snapshot
    ...
    for item in items:
        ...
        # Convert MiB to GiB dynamically if requested
        if mem_is_gi:
            mem_m = mem_m / 1024.0
    ...
    for ns_v, name, cpu_m, mem_val in rows:
        if mem_is_gi:
            lines.append(f"{ns_v:<{col_ns}}  {name:<{col_pod}}  {cpu_m:>8}m  {mem_val:>10.2f}{mem_label}")
        else:
            lines.append(f"{ns_v:<{col_ns}}  {name:<{col_pod}}  {cpu_m:>8}m  {mem_val:>10.0f}{mem_label}")
    lines.append("```")

    now_ts   = int(_time.time())
    sk_label = "CPU(m)" if sort_by.lower() in ("cpu", "cpu_m") else f"Memory({mem_label})"
    series_out = [
        {
...
def _get_top_pods_prometheus(namespace: str, limit: int, sort_by: str,
    ...
    def _make_series(ranked):
        return [
            {"label":  f"{ns_v}/{pod_v}" if ns_v != "-" else pod_v,
             "values": [[float(ts), float(v)] for ts, v in vals if str(v) not in ("NaN", "Inf", "-Inf", "+Inf")]}
            for ns_v, pod_v, _, _, vals in ranked
        ]
    ...
    if both_mode:
        ...
        if cpu_ranked:
            ...
            lines += [
                f"{direction.title()} {len(cpu_rows)} pods by peak CPU in {scope}{filter_note} — last {duration} ({from_str}–{to_str}, {step} interval).",
                "```",
                f"{'NAMESPACE':<{col_ns}}  {'POD':<{col_pod}}  {'PEAK (m)':>12}  {'LOWEST (m)':>12}",
                "-" * (col_ns + col_pod + 30),
            ]
            for ns_v, pod_v, peak_v, low_v in cpu_rows:
                lines.append(f"{ns_v:<{col_ns}}  {pod_v:<{col_pod}}  {peak_v:>10.1f}m  {low_v:>10.1f}m")
            
            # Print cap msg here ONLY if memory table is empty
            if not mem_ranked:
                lines.append("")
        ...
        if mem_ranked:
            ...
            lines += [
                f"{direction.title()} {len(mem_rows)} pods by peak memory in {scope}{filter_note} — last {duration} ({from_str}–{to_str}, {step} interval).",
                "```",
                f"{'NAMESPACE':<{col_ns}}  {'POD':<{col_pod}}  {'PEAK ('+mem_unit_label+')':>13}  {'LOWEST ('+mem_unit_label+')':>13}",
                "-" * (col_ns + col_pod + 32),
            ]
        ...
        graphs = []
        if cpu_ranked:
            graphs.append({"title": f"Top {len(cpu_ranked)} pods — CPU Usage ({from_str}–{to_str})",
                           "unit": "m", "duration": duration,
                           "series": _make_series(cpu_ranked)})
        if mem_ranked:
            graphs.append({"title": f"Top {len(mem_ranked)} pods — Memory Usage ({from_str}–{to_str})",
                           "unit": mem_unit_label, "duration": duration,
                           "series": _make_series(mem_ranked)})
    ...
    header  = (f"{direction.title()} {len(rows)} pods by peak {sort_key_label} "
               f"in {scope}{filter_note} — last {duration} ({from_str}–{to_str}, {step} interval).")
    col_ns  = max(len("NAMESPACE"), max(len(r[0]) for r in rows))
    col_pod = max(len("POD"),       max(len(r[1]) for r in rows))
    
    col_peak = f"PEAK({unit})"
    col_low  = f"LOWEST({unit})"
    
    hdr_row = f"{'NAMESPACE':<{col_ns}}  {'POD':<{col_pod}}  {col_peak:>14}  {col_low:>14}"
    sep     = "-" * len(hdr_row)
    lines   = [header, "```", hdr_row, sep]
    ...
    title      = f"{direction.title()} {len(rows)} pods — {sort_key_label} Usage ({from_str}–{to_str})"
    graph_json = _json.dumps(
...
def get_top_nodes(limit: int = 0, ascending: bool = False,
                  ...
                  user_timezone: str = "UTC", step: str = "10s", memory_unit: str = "Gi") -> str:
    import time as _time
    import datetime
    from datetime import timezone
    import json as _json
    ...
    if not duration:
        ...
        hdr_row  = f"{'NODE':<{col_node}}  {'CPU(m)':>9}  {'MEMORY('+mem_label+')':>12}"
        sep      = "-" * len(hdr_row)
        title    = (f"Node resource usage — {direction} {len(rows)} node(s) "
                    f"by {sort_key_label} — live at {now_str}.")
        lines    = [title, "```", hdr_row, sep]
        for name, cpu_m, mem_val in rows:
            if mem_is_gi:
                lines.append(f"{name:<{col_node}}  {cpu_m:>8}m  {mem_val:>10.2f}{mem_label}")
            else:
                lines.append(f"{name:<{col_node}}  {cpu_m:>8}m  {mem_val:>10.0f}{mem_label}")
        lines.append("```")

        now_ts     = int(_time.time())
        sk_label   = "CPU(m)" if sort_key_label == "CPU" else f"Memory({mem_label})"
        series_out = [
            {"label": name,
             "values": [[now_ts, float(cpu_m if sort_key_label == "CPU" else mem_val)]]}
            for name, cpu_m, mem_val in rows
        ]
        
        graph_json = _json.dumps(
            {"title": f"{direction.title()} {len(rows)} nodes — {sk_label} live",
             "unit":  "m" if sort_key_label == "CPU" else mem_label,
             "duration": "live", "series": series_out},
            separators=(",", ":"))
        return "\n".join(lines) + f"\n§GRAPH§{graph_json}§GRAPH§"
    ...
    return _get_top_nodes_prometheus(
        limit=limit, ascending=ascending, sort_by=sort_by, search="",
        duration=duration, user_timezone=user_timezone, 
        step=step, memory_unit=memory_unit)

def _get_top_nodes_prometheus(limit: int, ascending: bool, sort_by: str, 
                              search: str, duration: str, user_timezone: str,
                              step: str = "10s", memory_unit: str = "Gi") -> str:
    import time as _time
    import json as _json
    import urllib.parse
    import re
    ...
    def _peak_and_lowest(values: list) -> tuple[float, float]:
        vals = [float(v[1]) for v in values if str(v[1]) not in ("NaN", "Inf", "+Inf", "-Inf")]
        if not vals:
            return 0.0, 0.0
        return max(vals), min(vals)
    ...
    if disk_mode:
        ...
        write_results, write_err = _query_range(f'sum by (instance) (rate(node_disk_written_bytes_total[{rate_window}]))')
        if write_err:
            return f"Disk write query failed: {write_err}"

        read_by_inst  = {r.get("metric", {}).get("instance", "?"): (_peak_and_lowest(r.get("values", [])), r.get("values", [])) for r in read_results}
        write_by_inst = {r.get("metric", {}).get("instance", "?"): (_peak_and_lowest(r.get("values", [])), r.get("values", [])) for r in write_results}
        ...
        for inst in all_instances:
            ...
            if search and search.lower() not in node_name.lower():
                continue
            
            (read_peak, read_low), read_vals   = read_by_inst.get(inst,  ((0.0, 0.0), []))
            (write_peak, write_low), write_vals  = write_by_inst.get(inst, ((0.0, 0.0), []))
            rows.append((node_name, read_peak, write_peak, read_vals, write_vals))
        ...
        header   = (f"{direction.title()} {len(rows)} nodes by peak disk I/O "
                    f"— last {duration} ({from_str}–{to_str}, {step} interval).")
        col_node = max(len("NODE"), max(len(r[0]) for r in rows))
        hdr_row  = f"{'NODE':<{col_node}}  {'PEAK READ':>12}  {'PEAK WRITE':>12}"
        sep      = "-" * len(hdr_row)
        lines    = [header, "```", hdr_row, sep]
        for node_name, read_peak, write_peak, _, _ in rows:
            lines.append(f"{node_name:<{col_node}}  {_fmt_bytes(read_peak):>12}  {_fmt_bytes(write_peak):>12}")
        
        # Hardcoded 90d disclaimer inside the terminal block
        lines.append("")
        lines.append("  ⚠️ Note: Historical queries are safely capped at 90d to protect Prometheus.")
        lines.append("```")
        ...
        read_series  = [{"label": node_name,
                         "values": [[float(ts), float(v) / 1024**2]
                                    for ts, v in read_vals if str(v) not in _NAN]}
                        for node_name, _, _, read_vals, _ in rows]
        write_series = [{"label": node_name,
                         "values": [[float(ts), float(v) / 1024**2]
                                    for ts, v in write_vals if str(v) not in _NAN]}
    ...
    mem_results, mem_err = _query_range('sum by (instance) (node_memory_MemTotal_bytes - node_memory_MemAvailable_bytes)')
    if mem_err: return f"Memory query failed: {mem_err}"

    cpu_by_inst = {
        r.get("metric", {}).get("instance", "?"): (_peak_and_lowest(r.get("values", [])), r.get("values", []))
    ...
    for inst in all_instances:
        ...
        (cpu_peak, cpu_low), cpu_vals = cpu_by_inst.get(inst, ((0.0, 0.0), []))
        (mem_peak_b, mem_low_b), mem_vals = mem_by_inst.get(inst, ((0.0, 0.0), []))
        
        mem_peak = mem_peak_b / mem_divisor
        mem_low = mem_low_b / mem_divisor
        
        rows.append((node_name, cpu_peak, cpu_low, mem_peak, mem_low, cpu_vals, mem_vals))

    sort_idx = 3 if sort_key_label == "memory" else 1 # Index 3 is mem_peak, Index 1 is cpu_peak
    rows.sort(key=lambda x: x[sort_idx], reverse=not ascending)
    if limit > 0:
        rows = rows[:limit]

    header   = (f"{direction.title()} {len(rows)} nodes by peak {sort_key_label} "
                f"— last {duration} ({from_str}–{to_str}, {step} interval).")
    col_node = max(len("NODE"), max(len(r[0]) for r in rows))
    
    hdr_row  = f"{'NODE':<{col_node}}  {'PEAK CPU(c)':>12}  {'LOWEST CPU(c)':>14}  {f'PEAK MEM({mem_label})':>15}  {f'LOWEST MEM({mem_label})':>16}"
    sep      = "-" * len(hdr_row)
    lines    = [header, "```", hdr_row, sep]
    for node_name, cpu_peak, cpu_low, mem_peak, mem_low, _, _ in rows:
        lines.append(f"{node_name:<{col_node}}  {cpu_peak:>11.2f}c  {cpu_low:>13.2f}c  {mem_peak:>15.2f}  {mem_low:>16.2f}")
    ...
    cpu_series = [
        {"label":  node_name,
         "values": [[float(ts), float(v)]
                    for ts, v in cpu_vals if str(v) not in _NAN]}
        for node_name, _, _, _, _, cpu_vals, _ in rows
    ]
    mem_series = [
        {"label":  node_name,
         "values": [[float(ts), float(v) / mem_divisor]
                    for ts, v in mem_vals if str(v) not in _NAN]}
        for node_name, _, _, _, _, _, mem_vals in rows
    ]

    if sort_key_label == "memory":
        graphs = [
            {"title": f"Node memory usage ({mem_label}) — {from_str}–{to_str}",  "unit": mem_label,   "duration": duration, "series": mem_series},
...
def get_pods_using_resource(resource_type: str, resource_name: str, namespace: str = "all") -> str:
    if not resource_name.strip() or not resource_type.strip():
        return "[ERROR] Both resource_type and resource_name are required."
    
    r_type = resource_type.lower()
    if r_type not in ("secret", "configmap", "cm", "pvc", "persistentvolumeclaim"):
        return f"[ERROR] Unsupported resource type: {resource_type}. Use secret, configmap, or pvc."

    try:
        pods = _core.list_pod_for_all_namespaces().items if namespace == "all" else _core.list_namespaced_pod(namespace).items
        matched_pods = []
        ...
        for p in pods:
            ...
            for vol in (p.spec.volumes or []):
                if r_type == "secret" and vol.secret and vol.secret.secret_name == resource_name:
                    uses_resource = True
                elif r_type in ("configmap", "cm") and vol.config_map and vol.config_map.name == resource_name:
                    uses_resource = True
                elif r_type in ("pvc", "persistentvolumeclaim") and vol.persistent_volume_claim and vol.persistent_volume_claim.claim_name == resource_name:
                    uses_resource = True
                    
                if vol.projected and vol.projected.sources and r_type in ("secret", "configmap", "cm"):
                    for src in vol.projected.sources:
                        if r_type == "secret" and src.secret and src.secret.name == resource_name:
                            uses_resource = True
                        elif r_type in ("configmap", "cm") and src.config_map and src.config_map.name == resource_name:
                            uses_resource = True

            if not uses_resource and r_type in ("secret", "configmap", "cm"):
                for ctr in (p.spec.containers or []) + (p.spec.init_containers or []):
                    for env in (ctr.env or []):
                        if not env.value_from: continue
                        if r_type == "secret" and env.value_from.secret_key_ref and env.value_from.secret_key_ref.name == resource_name:
                            uses_resource = True
                        elif r_type in ("configmap", "cm") and env.value_from.config_map_key_ref and env.value_from.config_map_key_ref.name == resource_name:
                            uses_resource = True
                            
                    for env_from in (ctr.env_from or []):
                        if r_type == "secret" and env_from.secret_ref and env_from.secret_ref.name == resource_name:
                            uses_resource = True
                        elif r_type in ("configmap", "cm") and env_from.config_map_ref and env_from.config_map_ref.name == resource_name:
                            uses_resource = True
        ...
        scope = f"namespace `{namespace}`" if namespace != "all" else "all namespaces"
        if not matched_pods:
            return f"No pods found using {resource_type} `{resource_name}` in {scope}."
            
        lines = [f"### Pods using {resource_type} `{resource_name}` in {scope} ({len(matched_pods)} total)", 
                 "| NAMESPACE | POD NAME | STATUS |", 
                 "|---|---|---|"]
        for p in sorted(matched_pods, key=lambda x: (x.metadata.namespace, x.metadata.name)):
...
def _parse_prom_duration(duration: str, user_timezone: str = "UTC", max_days: int = 90) -> dict:
    import time as _time
    import re
    import datetime
    from datetime import timezone

    match = re.match(r"^(\d+)(s|m|h|d|w|mo|M|y)?$", duration.strip())
    if match:
        val = int(match.group(1))
        unit = match.group(2) or "h"
        mults = {"s": 1, "m": 60, "h": 3600, "d": 86400, "w": 604800, "mo": 2592000, "M": 2592000, "y": 31536000}
        dur_sec = val * mults.get(unit, 3600)
        
        if unit in ("mo", "M"): duration = f"{val * 30}d"
        elif unit == "w": duration = f"{val * 7}d"
    else:
        dur_sec = 3600
        duration = "1h"
    ...
    if dur_sec > max_sec:
        ...
        cap_msg = f"  ⚠️ Note: Historical queries are safely capped at {max_days}d to protect Prometheus."

    if dur_sec >= 86400 * 14:  step, rate_window = "1h", "1h"
    elif dur_sec >= 86400 * 3: step, rate_window = "30m", "30m"
    elif dur_sec >= 86400:     step, rate_window = "5m", "5m"
    elif dur_sec >= 3600 * 4:  step, rate_window = "1m", "2m"
    else:                      step, rate_window = "10s", "1m"
    ...
    try:
        import zoneinfo
        tz = zoneinfo.ZoneInfo(user_timezone)
        dt_start = datetime.datetime.fromtimestamp(start_ts, tz=timezone.utc).astimezone(tz)
        dt_end   = datetime.datetime.fromtimestamp(end_ts, tz=timezone.utc).astimezone(tz)
    except Exception:
        dt_start = datetime.datetime.utcfromtimestamp(start_ts)
        dt_end   = datetime.datetime.utcfromtimestamp(end_ts)
        tz_label = "UTC"

    if dur_sec >= 86400:
        from_str = dt_start.strftime("%b %d, %H:%M")
        to_str   = dt_end.strftime(f"%b %d, %H:%M {tz_label}")
    else:
        from_str = dt_start.strftime("%H:%M")
        to_str   = dt_end.strftime(f"%H:%M {tz_label}")

    return {
        "duration": duration, "dur_sec": dur_sec, "step": step, "rate_window": rate_window,
        "start_ts": start_ts, "end_ts": end_ts, "from_str": from_str, "to_str": to_str,
        "cap_msg": cap_msg
    }

def get_ingress_traffic(duration: str = "1h", user_timezone: str = "UTC") -> str:
    import json as _json
    import urllib.parse
    from kubernetes.stream import stream as _k8s_stream
    ...
    try:
        pods = _core.list_namespaced_pod(
            namespace="cdp",
            label_selector="app=prometheus,component=server").items
        for pod_obj in (pods or []):
            if pod_obj.status.phase == "Running":
                cnames = [c.name for c in (pod_obj.spec.containers or [])]
                prom_container = "prometheus-server" if "prometheus-server" in cnames else (cnames[0] if cnames else "prometheus-server")
                prom_pod       = pod_obj.metadata.name
                prom_ns        = pod_obj.metadata.namespace
                break
    except Exception as e:
        return f"[ERROR] Could not list CDP Prometheus pods: {e}"

    if not prom_pod:
        return (
            "No running CDP Prometheus pod found in namespace 'cdp' "
            "(expected label app=prometheus,component=server). "
            "Is cdp-release-prometheus-server running?"
        )
    ...
    if rx_err:
        return f"Ingress receive query failed: {rx_err}"

    tx_results, tx_err = _query_range(
        f'sum(rate(container_network_transmit_bytes_total'
        f'{{namespace="kube-system",pod=~"rke2-ingress-nginx-controller-.*"}}[{p["rate_window"]}]))')
    if tx_err:
        return f"Ingress transmit query failed: {tx_err}"

    if not rx_results and not tx_results:
        return (f"No ingress traffic data found in CDP Prometheus over the last {p['duration']}. "
                f"Prometheus pod: {prom_pod} in {prom_ns}.")

    rx_values  = rx_results[0].get("values", []) if rx_results else []
    tx_values  = tx_results[0].get("values", []) if tx_results else []
    
    rx_peak, rx_low = _peak_and_lowest(rx_values)
    tx_peak, tx_low = _peak_and_lowest(tx_values)

    def _fmt_speed(bps: float) -> str:
        if bps >= 1_048_576: return f"{bps / 1_048_576:.2f} MB/s"
        if bps >= 1_024:     return f"{bps / 1_024:.2f} KB/s"
        return f"{bps:.0f} B/s"

    # ── Summary table ─────────────────────────────────────────────────────────
    lines = [
        f"Ingress Nginx traffic — last {p['duration']} ({p['from_str']}–{p['to_str']}, {p['step']} interval).",
        "```",
        f"  {'DIRECTION':<10}  {'PEAK':>12}  {'LOWEST':>12}",
        f"  {'-'*10}  {'-'*12}  {'-'*12}",
        f"  {'Receive':<10}  {_fmt_speed(rx_peak):>12}  {_fmt_speed(rx_low):>12}",
        f"  {'Transmit':<10}  {_fmt_speed(tx_peak):>12}  {_fmt_speed(tx_low):>12}",
        "",
        f"  Prometheus: {prom_pod} (cdp)",
        "",
        "  ⚠️ Note: Historical queries are safely capped at 90d to protect Prometheus.",
        "```",
    ]
    ...
    if rx_values:
        graphs.append({
            "title":    f"Ingress Receive — last {p['duration']}",
            "unit":     "MB/s",
            "duration": p['duration'],
            "series":   [{"label": "Receive",
                          "values": [[float(v[0]), float(v[1]) / 1_048_576] for v in rx_values
                                     if str(v[1]) not in _NAN]}],
        })
    if tx_values:
        graphs.append({
            "title":    f"Ingress Transmit — last {p['duration']}",
            "unit":     "MB/s",
            "duration": p['duration'],
            "series":   [{"label": "Transmit",
                          "values": [[float(v[0]), float(v[1]) / 1_048_576] for v in tx_values
                                     if str(v[1]) not in _NAN]}],
        })
...
def get_longhorn_settings(search: str = None) -> str:
    try:
        custom = _k8s.CustomObjectsApi()
        # Try v1beta2 first, fall back to v1beta1 if older Longhorn version
        try:
            settings = custom.list_namespaced_custom_object(
                "longhorn.io", "v1beta2", "longhorn-system", "settings").get("items", [])
        except Exception:
            settings = custom.list_namespaced_custom_object(
                "longhorn.io", "v1beta1", "longhorn-system", "settings").get("items", [])

        if not settings:
            return "No Longhorn settings found in namespace 'longhorn-system'."

        lines = ["### Longhorn Settings (`longhorn-system`)",
                 "| NAME | VALUE |",
                 "|---|---|"]
        ...
        if search and not filtered:
            return f"No Longhorn settings found matching `{search}`."
...
def get_longhorn_node_status(node_name: str = None) -> str:
    try:
        custom = _k8s.CustomObjectsApi()
        try:
            items = custom.list_namespaced_custom_object(
                "longhorn.io", "v1beta2", "longhorn-system", "nodes"
            ).get("items", [])
        except ApiException as e:
            if e.status == 404:
                return "Longhorn is not installed on this cluster (nodes.longhorn.io CRD not found)."
            raise
        ...
        if node_name:
            ...
            if not items:
                return f"No Longhorn nodes found matching `{node_name}`."

        def _gib(b: int) -> str:
            return f"{b / (1024 ** 3):.1f}"

        def _cond(conditions: list, ctype: str) -> dict:
            return next((c for c in conditions if c.get("type") == ctype), {})

        node_lines = [
            "### Longhorn Node Status\n",
            "| NODE | READY | SCHEDULABLE | DISK PRESSURE | STORAGE MAX (GiB)"
            " | AVAIL (GiB) | SCHEDULED (GiB) | OVERCOMMIT % |",
            "|---|---|---|---|---|---|---|---|",
        ]

        for node in sorted(items, key=lambda n: n.get("metadata", {}).get("name", "")):
            name   = node.get("metadata", {}).get("name", "?")
            status = node.get("status", {})
            nconds = status.get("conditions", [])

            n_ready  = _cond(nconds, "Ready").get("status") == "True"
            n_sched  = _cond(nconds, "Schedulable").get("status") == "True"
            ready_fl = "✅" if n_ready else "🔴"
            sched_fl = "✅" if n_sched else "⚠️ Cordoned"

            disk_status = status.get("diskStatus", {})
            total_max = total_avail = total_sched = 0
            disk_pressure = False

            for disk_name, ds in disk_status.items():
                dconds = ds.get("conditions", [])
                if _cond(dconds, "Schedulable").get("reason") == "DiskPressure":
                    disk_pressure = True
                total_max   += ds.get("storageMaximum",  0)
                total_avail += ds.get("storageAvailable", 0)
                total_sched += ds.get("storageScheduled", 0)

            overcommit    = round((total_sched / total_max) * 100) if total_max > 0 else 0
            pressure_fl   = "🔴 Yes" if disk_pressure else "✅ No"
            overcommit_fl = (f"🔴 {overcommit}%" if overcommit >= 200
                             else f"🟠 {overcommit}%" if overcommit >= 100
                             else f"✅ {overcommit}%")

            node_lines.append(
                f"| `{name}` | {ready_fl} | {sched_fl} | {pressure_fl}"
                f" | {_gib(total_max)} | {_gib(total_avail)}"
                f" | {_gib(total_sched)} | {overcommit_fl} |"
            )
...
def get_pods_on_node(node_name: str) -> str:
    if not node_name.strip():
        return "[ERROR] You must provide a specific node_name."
    
    try:
        # Re-use the existing helper function that safely handles RKE2 WebSocket quirks
        pods = _list_pods_by_node(node_name)
        if not pods:
            return f"No pods are currently hosted on node `{node_name}`."
            
        lines = [f"### Pods hosted on node `{node_name}` ({len(pods)} total)",
                 "| NAMESPACE | POD NAME | STATUS | READY |",
                 "|---|---|---|---|"]
                 
        for p in sorted(pods, key=lambda x: (x.metadata.namespace, x.metadata.name)):
            ns    = p.metadata.namespace
            name  = p.metadata.name
            phase = p.status.phase or "Unknown"
            ready = sum(1 for cs in (p.status.container_statuses or []) if cs.ready)
            total = len(p.spec.containers)
            lines.append(f"| `{ns}` | `{name}` | {phase} | {ready}/{total} |")
            
        return "\n".join(lines)
    except Exception as e:
        return _k8s_err(e)
    
def get_crds(search: str = None) -> str:
    try:
        ext_api = _k8s.ApiextensionsV1Api()
        crds = ext_api.list_custom_resource_definition().items
        
        if not crds:
            return "No CustomResourceDefinitions (CRDs) found in the cluster."
            
        lines = ["### CustomResourceDefinitions (CRDs)",
                 "| NAME | GROUP | SCOPE | VERSIONS |",
                 "|---|---|---|---|"]
                 
        filtered = False
        for crd in sorted(crds, key=lambda x: x.metadata.name):
            ...
            versions = ", ".join(v.name for v in crd.spec.versions) if crd.spec.versions else "Unknown"
            
            lines.append(f"| `{name}` | `{group}` | {scope} | {versions} |")
            filtered = True
            
        if search and not filtered:
            return f"No CustomResourceDefinitions found matching `{search}`."
...
def run_cluster_health() -> str:
    ...
    try:
        ...
        for node in sorted(nodes, key=lambda n: n.metadata.name):
            ...
            if cordoned:
                status += " ⚠️  SchedulingDisabled"

            cpu_alloc     = _parse_cpu_cores(alloc.get("cpu", 0))
            pods_on_node  = _list_pods_by_node(node.metadata.name)
            cpu_req       = sum(
                _parse_cpu_cores((c.resources.requests or {}).get("cpu", 0))
                for p in pods_on_node for c in (p.spec.containers or []) if c.resources
            )
            cpu_used_pct  = round((cpu_req / cpu_alloc) * 100) if cpu_alloc > 0 else 0
            cpu_str       = f"{round(cpu_req, 2)}C / {round(cpu_alloc, 2)}C ({cpu_used_pct}% used)"
            ...
            mem_str       = f"{round(mem_req_gib, 1)}Gi / {round(mem_alloc_gib, 1)}Gi ({mem_used_pct}% used)"
            ...
            taints    = node.spec.taints or []
            taint_str = ", ".join(f"{t.key}{'=' + t.value if t.value else ''}:{t.effect}" for t in taints) if taints else "<none>"

            out.append(f"  🖥️  {node.metadata.name} | {status}{pressure_str} | CPU: {cpu_str} | RAM: {mem_str} | Taints: {taint_str}")

            if not ready:
                criticals.append(f"Node `{node.metadata.name}` is NotReady")
                node_issues = True
            if cordoned:
                warnings.append(f"Node `{node.metadata.name}` is cordoned (SchedulingDisabled)")
                node_issues = True
            if pressure:
                warnings.append(f"Node `{node.metadata.name}` has {', '.join(pressure)}")
                node_issues = True

        out.append(f"\n  📊 Summary: {schedulable}/{len(nodes)} node(s) schedulable")
        if not node_issues: healthy.append("Nodes")
    except Exception as e:
        out.append(f"⚠️  Could not check nodes: {e}")
        warnings.append("Node check failed")

    # Close the outer code block, emit etcd as rendered Markdown, then re-open
    out.append("```")
    out.append("\n── etcd " + "─" * 52)
    try:
        etcd_out = _get_etcd_metrics()
        out.append(etcd_out)
        if "unavailable" in etcd_out.lower():
            warnings.append("etcd metrics unavailable")
    except Exception as e:
        out.append(f"⚠️  Could not check etcd: {e}")
        warnings.append("etcd check failed")
    out.append("\n```")
    ...
    try:
        ...
        for ns in _SYSTEM_NAMESPACES:
            try:
                ...
                active_pods = [p for p in pods if p.status.phase not in ("Succeeded", "Completed")]
                unhealthy_pods = [
                    p for p in active_pods
                    if sum(1 for cs in (p.status.container_statuses or []) if cs.ready) < len(p.spec.containers or [])
                ]
                ...
                if unhealthy_pods:
                    ...
                    for p in unhealthy_pods:
                        ...
                        out.append(f"    🔴 Namespace: {ns} | Pod: {p.metadata.name} | Ready: {ready}/{tot_c} | Restarts: {restarts}{reason_str}")
                    warnings.append(f"{ns}: {len(unhealthy_pods)} pod(s) not ready")
                    sys_pod_issues = True
                else:
                    out.append(f"✅ {ns}: {total}/{total} pods ready")
            except Exception: pass
        if not sys_pod_issues: healthy.append("System Pods")
    except Exception as e:
    ...
    try:
        ...
        dep_bad = [f"{d.metadata.namespace}/{d.metadata.name}" for d in deps if (d.spec.replicas or 0) > 0 and (d.status.ready_replicas or 0) < (d.spec.replicas or 0)]
        if dep_bad:
        ...
        else: out.append(f"✅ {len(deps)} Deployments healthy")

        dss    = _apps.list_daemon_set_for_all_namespaces().items
        ds_bad = [f"{d.metadata.namespace}/{d.metadata.name}" for d in dss if (d.status.desired_number_scheduled or 0) > 0 and (d.status.number_ready or 0) < (d.status.desired_number_scheduled or 0)]
        if ds_bad:
            ...
            workload_issues = True
        else: out.append(f"✅ {len(dss)} DaemonSets healthy")

        stss    = _apps.list_stateful_set_for_all_namespaces().items
        sts_bad = [f"{s.metadata.namespace}/{s.metadata.name}" for s in stss if (s.spec.replicas or 0) > 0 and (s.status.ready_replicas or 0) < (s.spec.replicas or 0)]
        if sts_bad:
            ...
            workload_issues = True
        else: out.append(f"✅ {len(stss)} StatefulSets healthy")

        failed_pods = []
        for phase in _NOT_RUNNING_PHASES:
            try: failed_pods.extend(_core.list_pod_for_all_namespaces(field_selector=f"status.phase={phase}").items)
            except Exception: pass
        if failed_pods:
            ...
            for p in failed_pods:
                ...
                for cs in (p.status.container_statuses or []):
                    ...
                    if cs.state and cs.state.terminated and cs.state.terminated.reason:
                        reason = cs.state.terminated.reason; break
                reason_str = f" ({reason})" if reason else ""
                out.append(f"    🔴 Namespace: {p.metadata.namespace} | Pod: {p.metadata.name} | Phase: {phase} | Restarts: {restarts}{reason_str}")
            pod_labels = [f"{p.metadata.namespace}/{p.metadata.name}" for p in failed_pods]
            criticals.append(f"{len(failed_pods)} pod(s) not running: {', '.join(pod_labels[:5])}" + (f" (+{len(failed_pods)-5} more)" if len(failed_pods) > 5 else ""))
            workload_issues = True
        else:
            out.append("✅ No failed/pending pods")

        if not workload_issues: healthy.append("Workloads")
    except Exception as e:
    ...
    try:
        ...
        if unbound:
            ...
            criticals.append(f"{len(unbound)} PVC(s) not Bound: {', '.join(unbound[:5])}" + (f" (+{len(unbound)-5} more)" if len(unbound) > 5 else ""))
            storage_issues = True
        else:
    ...
    try:
        ...
        if over:
            flag_cnt = "🔴" if any(d["pct_val"] >= 90 for d in over) else "🟠"
            out.append(f"  {flag_cnt} Nearing or exceeding 80% capacity ({len(over)} PVC(s)):")
            out.append("```")
            # Markdown table for over-80% PVCs
            out.append("| FLAG | NAMESPACE / PVC | USAGE | USED (GiB) | TOTAL (GiB) | FREE (GiB) |")
            out.append("|---|---|---|---|---|---|")
            for d in sorted(over, key=lambda x: x["pct_val"], reverse=True):
                flag = "🔴" if d["pct_val"] >= 90 else "🟠"
                out.append(f"| {flag} | `{d['ns']}/{d['pvc_name']}` | **{d['pct_str']}** "
                           f"| {d['used_gib']}Gi ({d['pct_str']}) "
                           f"| {d['total_gib']}Gi "
                           f"| {d['avail_gib']}Gi ({d['free_pct']}%) |")
            out.append("```")
            if any(d["pct_val"] >= 90 for d in over):
                criticals.append(f"{sum(1 for d in over if d['pct_val'] >= 90)} PV(s) above 90% capacity")
            warnings.append(f"{len(over)} PV(s) above 80% capacity")
            storage_issues = True
        if under:
            out.append(f"✅ {len(under)} PVC(s) below 80% capacity")
        elif not over:
            out.append("✅ No PV usage data available")
    except Exception as e:
        out.append(f"⚠️  Could not check PV usage: {e}")
        warnings.append("PV usage check failed")
        storage_issues = True

    if not storage_issues: healthy.append("Storage")

    # ── Longhorn Node Status ──────────────────────────────────────────────────
    out.append("```")
    out.append("\n── Longhorn " + "─" * 48)
    try:
        lh_out = get_longhorn_node_status()
        if "not installed" in lh_out.lower():
            out.append("  ℹ️  Longhorn not detected on this cluster.")
        else:
            out.append(lh_out)
            if "🔴 Yes" in lh_out:
                warnings.append("Longhorn disk pressure detected on one or more nodes")
            if "⚠️ Cordoned" in lh_out:
                warnings.append("One or more Longhorn nodes are cordoned")
    except Exception as e:
        out.append(f"⚠️  Could not check Longhorn nodes: {e}")
        warnings.append("Longhorn node check failed")
    out.append("\n```")

    out.append("\n── Recent Warning Events " + "─" * 36)
    try:
        events = _core.list_event_for_all_namespaces(field_selector="type=Warning", limit=200).items
        seen: dict = {}
        for e in events:
            if _is_noisy_event(e.message or ""): continue
    ...
    out.append("\n── Summary of Notable Concerns " + "─" * 49)
    if criticals:
        out.append(f"🔴 {len(criticals)} critical issue(s):")
        for item in criticals: out.append(f"   • {item}")
    if warnings:
    ...
    out.append("")
    out.append("── 📋 What's Next? " + "─" * 41)
    out.append("• You may continue asking the chatbot to investigate any specific area reported above.")
...
def generate_healthcheck_report() -> str:
    ...
    # etcd deep metrics
    R.append(subsection("etcd Endpoint Status"))
    try:
        etcd_raw = _get_etcd_metrics()
        # _get_etcd_metrics returns Markdown — convert key parts to HTML table
        if "unavailable" in etcd_raw.lower():
            R.append(warn(etcd_raw.strip("_")))
        else:
            import ast as _ast
            from kubernetes.stream import stream as _k8s_stream
            # Re-exec to get structured data directly
            pods_ks = _core.list_namespaced_pod(namespace="kube-system").items
            etcd_pod = next(
                (p for p in pods_ks if p.metadata.name.startswith("etcd-")
                 and (p.status.phase or "") == "Running"), None)
            if etcd_pod:
                pod_name = etcd_pod.metadata.name
                tls_cert = tls_key = tls_ca = None
                for container in (etcd_pod.spec.containers or []):
                    if container.name != "etcd": continue
                    args = list(container.command or []) + list(container.args or [])
                    for arg in args:
                        if arg.startswith("--cert-file="):     tls_cert = arg.split("=",1)[1]
                        elif arg.startswith("--key-file="):    tls_key  = arg.split("=",1)[1]
                        elif arg.startswith("--trusted-ca-file="): tls_ca = arg.split("=",1)[1]
                RKE2_BASE = "/var/lib/rancher/rke2/server/tls/etcd"
                if not tls_cert:
                    tls_cert = f"{RKE2_BASE}/server-client.crt"
                    tls_key  = f"{RKE2_BASE}/server-client.key"
                    tls_ca   = f"{RKE2_BASE}/server-ca.crt"
                cmd = ["etcdctl", "--endpoints=https://127.0.0.1:2379",
                       f"--cacert={tls_ca}", f"--cert={tls_cert}", f"--key={tls_key}",
                       "endpoint", "status", "-w", "json"]
                try:
                    raw = _k8s_stream(_core.connect_get_namespaced_pod_exec,
                                      pod_name, "kube-system", command=cmd, container="etcd",
                                      stderr=True, stdin=False, stdout=True, tty=False,
                                      _preload_content=True)
                    raw = (raw or "").strip()
                    payload = raw[raw.find("["):]
                    try:    data = _json.loads(payload)
                    except: data = _ast.literal_eval(payload)
                    if data:
                        def _fb(b):
                        ...
                        for entry in data:
                            st  = entry.get("Status", {})
                            hdr = st.get("header", {})
                            mid = hdr.get("member_id", 0)
                            is_leader = mid == st.get("leader", -1)
                            leader_html = ('<span style="color:#16a34a;font-weight:700">&#10003; Leader</span>'
                                           if is_leader else "Follower")
                            rows.append([
                                esc(entry.get("Endpoint","?")),
                                esc(format(mid, "x")),
                                esc(st.get("version","?")),
                                esc(_fb(st.get("dbSize",0))),
                                esc(_fb(st.get("dbSizeInUse",0))),
                                leader_html,
                                esc(str(st.get("raftTerm", hdr.get("raft_term","?")))),
                                esc(f"{st.get('raftIndex','?'):,}"),
                                esc(str(st.get("errors","") or "—")),
                            ])
                        R.append(table(["ENDPOINT","MEMBER ID","VERSION","DB SIZE",
                                        "DB IN USE","ROLE","RAFT TERM","RAFT INDEX","ERRORS"], rows))
                        st0  = data[0].get("Status", {})
                        hdr0 = st0.get("header", {})
                        R.append(f'<p style="font-size:11px;color:#64748b">'
                                 f'Cluster ID: <code>{format(hdr0.get("cluster_id",0),"x")}</code> &nbsp;·&nbsp; '
                                 f'Revision: <code>{hdr0.get("revision","?"):,}</code> &nbsp;·&nbsp; '
                                 f'Pod: <code>{pod_name}</code></p>')
                except Exception as ex:
                    R.append(warn(f"etcd exec failed: {_report_err(ex)}"))
            else:
                R.append(warn("No running etcd pod found in kube-system"))
    except Exception as e:
        R.append(warn(f"etcd check failed: {_report_err(e)}"))
    ...
    # Longhorn node status
    R.append(subsection("Longhorn Node Status"))
    try:
        custom = _k8s.CustomObjectsApi()
        try:
            lh_items = custom.list_namespaced_custom_object(
                "longhorn.io", "v1beta2", "longhorn-system", "nodes"
            ).get("items", [])
            lh_installed = True
        ...
        if not lh_installed:
            R.append(ok("Longhorn is not installed on this cluster."))
        elif not lh_items:
            R.append(warn("No Longhorn nodes found in longhorn-system."))
        else:
            def _gib(b): return f"{b/(1024**3):.1f}"
            def _lhcond(conds, ctype):
                return next((c for c in conds if c.get("type") == ctype), {})
            rows = []
            for node in sorted(lh_items, key=lambda n: n.get("metadata",{}).get("name","")):
                name   = node.get("metadata",{}).get("name","?")
                status = node.get("status",{})
                nconds = status.get("conditions",[])
                n_ready = _lhcond(nconds,"Ready").get("status") == "True"
                n_sched = _lhcond(nconds,"Schedulable").get("status") == "True"
                ready_html = ('<span style="color:#16a34a;font-weight:700">&#10003; Ready</span>'
                              if n_ready else '<span style="color:#dc2626;font-weight:700">&#10007; NotReady</span>')
                sched_html = ('&#10003;' if n_sched
                              else '<span style="color:#d97706;font-weight:700">&#9888; Cordoned</span>')
                disk_status = status.get("diskStatus",{})
                total_max = total_avail = total_sched = 0
                disk_pressure = False
                for dk, ds in disk_status.items():
                    if _lhcond(ds.get("conditions",[]),"Schedulable").get("reason") == "DiskPressure":
                        disk_pressure = True
                    total_max   += ds.get("storageMaximum",  0)
                    total_avail += ds.get("storageAvailable", 0)
                    total_sched += ds.get("storageScheduled", 0)
                overcommit = round((total_sched / total_max) * 100) if total_max > 0 else 0
                pressure_html = ('<span style="color:#dc2626;font-weight:700">&#9679; Yes</span>'
                                 if disk_pressure else '&#10003; No')
                oc_color = "#dc2626" if overcommit >= 200 else ("#d97706" if overcommit >= 100 else "#16a34a")
                oc_html  = f'<span style="color:{oc_color};font-weight:700">{overcommit}%</span>'
                rows.append([esc(name), ready_html, sched_html, pressure_html,
                             esc(_gib(total_max)), esc(_gib(total_avail)),
                             esc(_gib(total_sched)), oc_html])
            R.append(table(["NODE","READY","SCHEDULABLE","DISK PRESSURE",
                             "MAX (GiB)","AVAIL (GiB)","SCHEDULED (GiB)","OVERCOMMIT %"], rows))
    except Exception as e:
        R.append(warn(f"Could not gather Longhorn node data: {_report_err(e)}"))
...
def _get_etcd_metrics() -> str:
    from kubernetes.stream import stream as _k8s_stream

    # ── 1. Locate the etcd pod in kube-system ────────────────────────────────
    try:
        pods = _core.list_namespaced_pod(namespace="kube-system").items
    except Exception as e:
        return f"_(etcd metrics unavailable: could not list kube-system pods — {e})_"

    etcd_pod = next(
        (p for p in pods if p.metadata.name.startswith("etcd-") and
         (p.status.phase or "") == "Running"),
        None,
    )
    if etcd_pod is None:
        return "_(etcd metrics unavailable: no running etcd pod found in kube-system)_"

    pod_name  = etcd_pod.metadata.name
    namespace = "kube-system"

    # ── 2. Detect TLS cert paths (RKE2 vs kubeadm) ──────────────────────────
    # Read the etcd container's command/args to find --cert-file etc.
    tls_cert = tls_key = tls_ca = None
    for container in (etcd_pod.spec.containers or []):
        ...
        for arg in args:
            ...
            elif arg.startswith("--trusted-ca-file="):
                tls_ca = arg.split("=", 1)[1]

    # Fallback to known RKE2 paths if args parsing didn't yield paths
    RKE2_BASE    = "/var/lib/rancher/rke2/server/tls/etcd"
    KUBEADM_BASE = "/etc/kubernetes/pki/etcd"
    if not tls_cert:
        tls_cert = f"{RKE2_BASE}/server-client.crt"
        tls_key  = f"{RKE2_BASE}/server-client.key"
        tls_ca   = f"{RKE2_BASE}/server-ca.crt"

    # ── 3. Exec etcdctl endpoint status -w json ──────────────────────────────
    cmd = [
        "etcdctl",
    ...
    try:
        ...
        if RKE2_BASE in tls_cert:
            tls_cert = f"{KUBEADM_BASE}/server.crt"
            tls_key  = f"{KUBEADM_BASE}/server.key"
            tls_ca   = f"{KUBEADM_BASE}/ca.crt"
            cmd[3] = f"--cacert={tls_ca}"
            cmd[4] = f"--cert={tls_cert}"
            cmd[5] = f"--key={tls_key}"
            try:
                raw = _k8s_stream(
                    _core.connect_get_namespaced_pod_exec,
                ...
                return f"_(etcd metrics unavailable: exec failed — {e2})_"
        else:
            return f"_(etcd metrics unavailable: exec failed — {e})_"
    ...
    start = raw.find("[")
    if start == -1:
        return f"_(etcd metrics unavailable: unexpected output — {raw[:200]})_"
    payload = raw[start:]
    try:
        ...
        try:
            ...
            return f"_(etcd metrics unavailable: could not parse output — {e}; raw: {raw[:200]})_"

    if not data:
        return "_(etcd metrics unavailable: empty response from etcdctl)_"

    # ── 5. Format output ─────────────────────────────────────────────────────
    lines = [
        "",
        "**etcd Endpoint Status:**",
        "| ENDPOINT | MEMBER ID | VERSION | DB SIZE | DB SIZE IN USE | IS LEADER | RAFT TERM | RAFT INDEX | ERRORS |",
        "|---|---|---|---|---|---|---|---|---|",
    ]
    for entry in data:
        ...
        st        = entry.get("Status", {})
        hdr       = st.get("header", {})
        member_id = format(hdr.get("member_id", 0), "x")  # hex, like etcdctl
        version   = st.get("version", "?")
        db_bytes  = st.get("dbSize", 0)
        db_inuse  = st.get("dbSizeInUse", 0)
        leader_id = st.get("leader", 0)
        is_leader = "✅ true" if hdr.get("member_id") == leader_id else "false"
        raft_term = st.get("raftTerm", hdr.get("raft_term", "?"))
        raft_index = st.get("raftIndex", "?")
        ...
        lines.append(
            f"| `{endpoint}` | `{member_id}` | {version} "
            f"| {_fmt_bytes(db_bytes)} | {_fmt_bytes(db_inuse)} "
            f"| {is_leader} | {raft_term} | {raft_index:,} | {errors} |"
        )
    ...
    if data:
        ...
        lines += [
            "",
            "**etcd Cluster Info:**",
            f"- Cluster ID: `{format(hdr.get('cluster_id', 0), 'x')}`",
            f"- Revision: `{hdr.get('revision', '?'):,}`",
            f"- Pod: `{pod_name}` (kube-system)",
        ]
...
def get_control_plane_status() -> str:
    try:
        ...
        # ── etcd deep metrics ─────────────────────────────────────────────────
        lines.append("")
        lines.append(_get_etcd_metrics())
                ...
                for sname in sorted(secrets):
                    try:
                        ...
                        for k, v in data.items():
                            ...
                            lines.append(f"      {k}: {_decode(v) if decode else hidden_msg}")
                    except ApiException:
