...
def build_agent():
    ...
    def _prepare_messages_for_hf(msgs: list, req_id: str = "") -> list:
        ...
        _TOOL_FORMATS = { # Unique prompt for individual tool
            "get_node_taints": (
                "The table above is the complete and final taint data. Do NOT call any tool. "
                "Answer the user's question directly from this table. "
                "The table lists every tainted node with its taint key, value, and effect. "
                "If the user asks which nodes are tainted, list them from this table only."
            ),
            "get_node_info": (
                "The table above is the complete and final node data. Do NOT call any tool. "
                "Answer the user's question directly from this table. "
                "IMPORTANT: SchedulingDisabled (cordoned) means NO NEW pods can be scheduled on that node — "
                "it does NOT mean the node has no pods. Existing pods continue running normally on a cordoned node. "
                "Never say a cordoned node has no pods unless you have confirmed this from pod data."
            ),
            "get_gpu_info": (
            ...
            "get_pods_using_resource": (
                "Reproduce the table output VERBATIM. "
                "Do NOT reformat, summarise, or omit any rows or headers."
            ),
            ...
            "get_node_capacity": (
                ...
                "Do NOT reproduce the raw table. Instead, perform a fit-check against the user's requested numbers.\n"
    ...
    def tool_node(state: AgentState):
        ...
        for tc in tcs:
            ...
            # Only force the namespace if the LLM defaulted to "all" or left it blank
            if args.get("namespace") in (None, "", "all", "default") and forced_ns != "all":
            ...
            _log_str = re.sub(
                r'§GRAPH§(.*?)§GRAPH§', 
                lambda m: f"§GRAPH§{m.group(1)[:10]}... end of §GRAPH§", 
                _out_str, 
                flags=re.DOTALL
            )
            if tool_names == "generate_healthcheck_report" and len(_log_str) > 300:
                _log_str = _log_str[:300] + "\n\n... [WEASYPRINT HTML TRUNCATED FOR LOGS]"
            ...
            if name == "rag_search" and isinstance(out, str) and out.startswith("KB_EMPTY:"):
                ...
                direct_answer = out + (BYPASS_NOTE if name in UNCONDITIONAL_BYPASS else "")
...
async def run_agent_streaming(user_message: str, history: list = None, max_new_tokens: int = 0, skip_synthesise: bool = False):
    ...
    try:
        ...
        if not _final_cleaned.strip():
            config.logger.warning(f"[REQ:{req_id}] Something wrong! Falling back to raw tool output.")
        ...
        import re
        _log_final_cleaned = re.sub(
            r'§GRAPH§(.*?)§GRAPH§', 
            lambda m: f"§GRAPH§{m.group(1)[:50]}... end of §GRAPH§", 
            _final_cleaned, 
            flags=re.DOTALL
        )
        ...
        _hb_stop.set(); _hb_task.cancel()
        config.logger.info(f"[REQ:{req_id}] 🛑 Generation stopped by user (client disconnected).")
        raise
...
def _llm_synthesise(context: str, question: str, top_k: int = 50, max_tokens: int = 0) -> str:
    ...
    if kb_prompt_path.exists():
        ...
        sys_prompt = (
            "You are the ECS Incident & KB Bot for Cloudera ECS. Answer questions using ONLY the provided Knowledge Base Context.\n"
            "1. VERBATIM STEPS: Reproduce numbered steps, remediation procedures, and commands EXACTLY as written — do not paraphrase or condense.\n"
            "2. MISSING DATA: If the context says 'KB_EMPTY' or 'No relevant documentation', tell the user the knowledge base is empty and direct them to ⚙ Settings → RAG Documents.\n"
            "3. CHIT-CHAT: If the user greets you or asks what you can do, introduce yourself and say you can search for Incidents, Known Issues, and Dos & Don'ts.\n"
            "4. NO PREAMBLE: Deliver answers directly without filler phrases."
        )
    ...
    #_max_out = config.MAX_NEW_TOKENS
    _max_out = 8192
...
async def chat_stream(req: ChatRequest, request: Request):
    ...
    async def _keepalive_stream():
        ...
        async def _producer():
            try:
                async for chunk in run_agent_streaming(req.message, req.history, req.max_new_tokens, req.skip_synthesise): 
        ...
        try:
            while True:
                # Instantly detect if the user clicked the Stop! button
...
async def ingest_upload_real(files: list[UploadFile] = File(...), force: str = FastAPIForm(default="false")):
    ...
    for upload in files:
        ...
        fhash = _hashlib.md5(content).hexdigest()
        dest  = docs_dir / upload.filename

        # Check for duplicate BEFORE writing to disk
        if not do_force:
            try:
                from rag.store import _get_lancedb
                _, docs_tbl, excel_tbl = _get_lancedb()
                tbl = excel_tbl if suffix in (".xlsx", ".xls") else docs_tbl
                field = "source_file" if suffix in (".xlsx", ".xls") else "source"
                val   = upload.filename if suffix in (".xlsx", ".xls") else str(dest)
                safe_val = val.replace("'", "''")
                safe_hash = fhash.replace("'", "''")
                existing = tbl.search().where(
                    f"{field} = '{safe_val}' AND file_hash = '{safe_hash}'"
                ).limit(1).to_list()
                if existing:
                    results.append({"file": upload.filename, "status": "skipped", "chunks": 0})
                    continue
            except Exception:
                pass  # If check fails, proceed with ingest
...
def _fetch_report_charts() -> dict:
    ...
    # ── Ingress traffic via CDP Prometheus ────────────────────────────────────
    cdp_prom_pod = cdp_prom_ns = cdp_prom_container = None
    try:
        cdp_pods = _tk._core.list_namespaced_pod(
            namespace="cdp",
            label_selector="app=prometheus,component=server").items
        for p in (cdp_pods or []):
            if p.status.phase == "Running":
                cnames             = [c.name for c in (p.spec.containers or [])]
                cdp_prom_container = ("prometheus-server" if "prometheus-server" in cnames
                                      else (cnames[0] if cnames else "prometheus-server"))
                cdp_prom_pod       = p.metadata.name
                cdp_prom_ns        = p.metadata.namespace
                break
    except Exception:
        pass
    ...
    if cdp_prom_pod:
        def _exec_cdp(cmd):
            try:
                ws = _k8s_stream(
                    _tk._core.connect_get_namespaced_pod_exec,
                    cdp_prom_pod, cdp_prom_ns,
                    command=["/bin/sh", "-c", cmd],
                    container=cdp_prom_container,
                    stderr=False, stdin=False, stdout=True, tty=False,
                    _preload_content=False)
                chunks = []
                while ws.is_open():
        ...
        cdp_api_base = "http://localhost:9090/api/v1"

        def _query_range_cdp(pql, start, end, step):
            enc = urllib.parse.quote(pql, safe="")
            url = f"{cdp_api_base}/query_range?query={enc}&start={start}&end={end}&step={step}"
            raw = _exec_cdp(f"curl -s --max-time 120 '{url}'")
            if not raw or raw.startswith("[exec error"):
                return []
            try:
                data = _json.loads(raw)
            ...
            series = []
            for r in results:
                vals = [[float(ts), float(v) / 1_048_576]
                        for ts, v in r.get("values", [])
                        if v not in ("NaN", "Inf", "+Inf", "-Inf")]
                if vals:
                    label = "Receive" if "receive" in pql else "Transmit"
                    series.append({"label": label, "values": vals})
            return series

        ingress_rx_pql = ('sum(rate(container_network_receive_bytes_total'
                          '{namespace="kube-system",pod=~"rke2-ingress-nginx-controller-.*"}[5m]))')
        ingress_tx_pql = ('sum(rate(container_network_transmit_bytes_total'
                          '{namespace="kube-system",pod=~"rke2-ingress-nginx-controller-.*"}[5m]))')
        ...
        for win, (start, end, step) in ingress_windows.items():
            result[f"ingress_rx_{win}"] = _query_range_cdp(ingress_rx_pql, start, end, step)
            result[f"ingress_tx_{win}"] = _query_range_cdp(ingress_tx_pql, start, end, step)
...
async def api_rag_files():
    """List ingested files from LanceDB — returns objects matching frontend shape."""
    try:
        _, docs_tbl, excel_tbl = _rag.get_doc_stats.__wrapped__() if hasattr(_rag.get_doc_stats, '__wrapped__') else (None, None, None)
    except Exception:
        pass
    ...
    try:
        ...
        # Docs (pdf/md/txt) — distinct sources from LanceDB
        try:
            from collections import defaultdict
            docs_rows = docs_tbl.search().limit(10000).to_list()
            by_source: dict = defaultdict(lambda: {"chunks": 0, "doc_type": "general"})
            for r in docs_rows:
                src = r.get("source", "")
                by_source[src]["chunks"]   += 1
                by_source[src]["doc_type"]  = r.get("doc_type", "general")
            for src, meta in by_source.items():
                name   = Path(src).name
                suffix = Path(name).suffix.lower()
                ftype  = "pdf" if suffix == ".pdf" else "markdown" if suffix == ".md" else "text"
                files_out.append({
                    "filename": name,
                    "type":     ftype,
                    "chunks":   meta["chunks"],
        ...
        # Excel — distinct source_file from LanceDB
        try:
            from collections import defaultdict, Counter
            excel_rows = excel_tbl.search().limit(10000).to_list()
            by_file: dict = defaultdict(lambda: {"rows": 0, "sheets": set()})
            for r in excel_rows:
                sf = r.get("source_file", "")
                by_file[sf]["rows"]   += 1
                by_file[sf]["sheets"].add(r.get("sheet", ""))
            for sf, meta in by_file.items():
                files_out.append({
                    "filename": sf,
                    "type":     "excel",
                    "rows":     meta["rows"],
                    "sheets":   sorted(s for s in meta["sheets"] if s),
                })
        except Exception:
            pass
    ...
    files_out.sort(key=lambda f: f["filename"])
    return {"count": len(files_out), "files": files_out}


@app.delete("/api/rag/reset")
async def api_rag_reset():
    """Purge all ingested data from LanceDB tables and the docs directory."""
    import shutil
    errors = []
    try:
        from rag.store import _get_lancedb
        _, docs_tbl, excel_tbl = _get_lancedb()
        try:
            docs_tbl.delete("id IS NOT NULL")
        except Exception:
            try: docs_tbl.delete("chunk_index >= 0")
            except Exception as e: errors.append(f"docs_tbl: {e}")
        try:
            excel_tbl.delete("id IS NOT NULL")
        except Exception as e:
            errors.append(f"excel_tbl: {e}")
    except Exception as e:
    ...
    config.logger.info(f"[RAG/reset] Purged {len(deleted_files)} files, errors={errors}")
    return {
        "ok":      len(errors) == 0,
...
async def api_rag_delete_file(filename: str):
    """Delete a single ingested file — removes its vectors from LanceDB and the file from disk."""
    ...
    try:
        ...
        # Try docs table — match by source path ending with filename
        try:
            docs_dir   = config._HERE / "docs"
            full_path  = str(docs_dir / filename)
            before     = docs_tbl.count_rows()
            docs_tbl.delete(f"source = '{full_path}'")
            after      = docs_tbl.count_rows()
            vectors_deleted += max(0, before - after)
        except Exception as e:
            errors.append(f"docs_tbl: {e}")

        # Try excel table — match by source_file (basename only)
        try:
            before = excel_tbl.count_rows()
            safe   = filename.replace("'", "''")
            excel_tbl.delete(f"source_file = '{safe}'")
            after  = excel_tbl.count_rows()
            vectors_deleted += max(0, before - after)
        except Exception as e:
            errors.append(f"excel_tbl: {e}")
    ...
    # Remove physical file from docs/
    try:
        docs_dir  = config._HERE / "docs"
        ...
        errors.append(f"file_unlink: {e}")

    config.logger.info(
        f"[RAG/delete-file] filename={filename!r} vectors_deleted={vectors_deleted} "
        f"file_deleted={file_deleted} errors={errors}"
    )
    return {
        "ok":              len(errors) == 0,
        "filename":        filename,
...
async def api_kb_stream(req: KbAskRequest, request: Request):
    ...
    async def _generate():
        ...
        try:
            ...
            # Check if user disconnected during DB search
            if await request.is_disconnected():
                config.logger.info(f"[KB] 🛑 Search stopped by user before LLM synthesis.")
                return
            ...
            # Poll the connection every 0.5s while the GPU churns
            while not llm_task.done():
                if await request.is_disconnected():
                    config.logger.info(f"[KB] 🛑 Search stopped by user. (Note: GPU will finish the orphaned task in the background).")
                    return
                await _asyncio.sleep(0.5)

            answer = llm_task.result()
            ...
            config.logger.info(f"[KB] 🛑 Search cancelled by client disconnect.")
            raise
        except Exception as exc:
            config.logger.error(f"[KB Stream Error] {exc}", exc_info=True)
            yield _sse({"type": "error", "text": str(exc)})
